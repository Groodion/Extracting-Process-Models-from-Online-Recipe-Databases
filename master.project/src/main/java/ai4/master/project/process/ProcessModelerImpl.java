            ...
            if (!idExists(createIdOf(node.getData().getText()))) { //Avoid duplicates by having more than one dependence which creates two parts in the tree.
                ...
                if(this.withOutput) {
	                for (Ingredient ingredient : node.getData().getProducts()) {
	                	 System.out.println("Creating dataObject_I" + i + "_" + createIdOf(ingredient.getCompleteName()));
	                	 String id = createIdOf("dataObject_I" + i + "_" + createIdOf(ingredient.getCompleteName()) + createIdOf(node.getData().getText()));
                        i++;

	                     DataObjectReference dor = createDataObject(process, id, ingredient.getName(), plane, true);
	                     dataObjects.add(dor);
