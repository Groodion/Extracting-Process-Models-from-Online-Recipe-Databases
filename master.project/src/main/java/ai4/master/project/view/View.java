...
public class View extends Application {
	...
	private ObservableList<String> problemsEntry = FXCollections.observableArrayList(
			"Unknown Child Group in groups",
			"Unknown Child Group in groups",
			"Unknown Child Group in groups",
			"Unknown Child Group in groups"
			);
		...
		chefkoch.setOnMouseClicked(new EventHandler<MouseEvent>() {
			...
			public void handle(MouseEvent e) {
				...
				if (result.isPresent()){
				   ...
				   recipeText.setText(recipeChefkochGetter.getRecipe(id).toString());
				}
			}
